...
export class AdminController {
  ...
  static async getAllEventPlanners(req: Request, res: Response) {
    try {
      ...
      const plannersResult = await db
        ...
        .from(eventPlanners)
        .leftJoin(user, eq(eventPlanners.profileId, user.id))
        .where(finalWhere)
        .orderBy(orderFunc(sortColumn))
        .limit(limitNum)
        .offset(offset);

      const planners = plannersResult.map(({ planner, owner }) => ({
        ...planner,
        owner: owner ?? null,
      }));
  ...
  static async toggleEventPlannerVerification(req: Request, res: Response) {
    try {
      ...
      console.error("Error toggling event planner verification:", error);
