"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const express_1 = require("express");
const usersRoutes_1 = __importDefault(require("./usersRoutes"));
const chats_1 = __importDefault(require("./chats"));
const eventPlanner_1 = __importDefault(require("./eventPlanner"));
const vendor_services_1 = __importDefault(require("./vendor-services"));
const vendor_1 = __importDefault(require("./vendor"));
const event_1 = __importDefault(require("./event"));
const tickets_1 = __importDefault(require("./tickets"));
const settings_1 = __importDefault(require("./settings"));
const auth_1 = __importDefault(require("./auth"));
const payments_1 = __importDefault(require("./payments"));
const orders_1 = __importDefault(require("./orders"));
const bookings_1 = __importDefault(require("./bookings"));
const admin_1 = __importDefault(require("./admin"));
const discount_codes_1 = __importDefault(require("./discount-codes"));
const router = (0, express_1.Router)();
// app.ts / index.ts
router.use("/bookings", bookings_1.default);
router.use("/users", usersRoutes_1.default);
router.use("/orders", orders_1.default);
router.use("/auth", auth_1.default);
router.use("/wallets", payments_1.default);
router.use("/vendors", vendor_1.default);
router.use("/vendor-services", vendor_services_1.default);
router.use("/settings", settings_1.default);
router.use("/chats", chats_1.default);
router.use("/tickets", tickets_1.default);
router.use("/events", event_1.default);
router.use("/event-planners", eventPlanner_1.default);
router.use("/admin", admin_1.default);
router.use("/discount-codes", discount_codes_1.default);
exports.default = router;
